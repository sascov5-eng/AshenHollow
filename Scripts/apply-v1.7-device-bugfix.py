...
newplat='''    private func updateMovingPlatforms(_ dt: TimeInterval) {
        platformContactGrace = max(0, platformContactGrace - dt); let pf=playerColliderFrame(); var contacted:String?
        for (id,c) in movingControllers { let old=c.frame; let d=c.update(dt:dt); movingNodes[id]?.position=c.state.position; let overlap=pf.maxX>old.minX+6 && pf.minX<old.maxX-6; let gap=pf.minY-old.maxY; let landing=overlap && gap >= -8 && gap <= 14 && velocity.dy <= 40; let retained=riderPlatformID==id && platformContactGrace>0 && overlap && gap >= -14 && gap <= 22 && velocity.dy <= 80; if landing || retained { contacted=id; riderPlatformID=id; platformContactGrace=0.14; player.position.x += d.dx; player.position.y += d.dy; if velocity.dy <= 0 { isGrounded=true }; tutorialController.register(action:c.spec.axis == .horizontal ? .movingPlatformHorizontal:.movingPlatformVertical) } }
...
insert='''    private func isSafeTrialSpawn(_ point: CGPoint) -> Bool { let f=CGRect(x:point.x-18,y:point.y-30,width:36,height:60); return !worldLayout.hazards.contains(where:{$0.rect.insetBy(dx:-24,dy:-18).intersects(f)}) }
    private func safeSpawnPosition() -> CGPoint { let direct=staticPlatformRects.compactMap { r -> CGPoint? in guard trialDefinition.startX>=r.minX+42 && trialDefinition.startX<=r.maxX-42 else{return nil};let p=CGPoint(x:trialDefinition.startX,y:r.maxY+30);return isSafeTrialSpawn(p) ? p:nil }.sorted{abs($0.y-trialDefinition.spawnHintY)<abs($1.y-trialDefinition.spawnHintY)};if let p=direct.first,abs(p.y-trialDefinition.spawnHintY)<=trialDefinition.safeSpawnRadius{return p};for off in stride(from:CGFloat(40),through:trialDefinition.safeSpawnRadius,by:40){for x in [trialDefinition.startX-off,trialDefinition.startX+off]{for r in staticPlatformRects where x>=r.minX+42 && x<=r.maxX-42{let p=CGPoint(x:x,y:r.maxY+30);if isSafeTrialSpawn(p){return p}}}};return worldLayout.spawnPoint }'''
